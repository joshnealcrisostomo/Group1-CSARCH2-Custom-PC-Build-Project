...
A significant portion of the budget was allocated to the processor, since it has the biggest impact on overall performance. The group decided not to purchase a dedicated graphics card because of budget constraints, but we made sure to choose a CPU with integrated graphics that can handle light gaming, productivity tasks, and programming. To keep costs manageable, we also picked reliable yet affordable options for the RAM, storage, and motherboard. Additional savings came from using the stock cooler that came with the CPU and choosing a more budget-friendly PC case, which freed up funds for other key parts. At the same time, we selected a slightly higher-wattage power supply to allow for possible upgrades in the future.
